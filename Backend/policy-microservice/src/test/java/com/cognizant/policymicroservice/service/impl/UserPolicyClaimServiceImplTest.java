...
class UserPolicyClaimServiceImplTest {
	...
	@Test
	void testViewClaim() {
		PolicyMaster pObj = new PolicyMaster(1L, "POL_1", "medical", 100, 1000);
		UserPolicyClaim claimObj = new UserPolicyClaim(1L, new Date(), 5000, "success", new Date(), pObj);
		when(userPolicyClaimRepo.findAll()).thenReturn(Stream
				.of(new UserPolicyClaim(1L, new Date(), 5000, "success", new Date(), pObj),
						new UserPolicyClaim(2L, new Date(), 5000, "success", new Date(), pObj))
				.collect(Collectors.toList()));
		assertThat(service.viewClaim().size()).isEqualTo(2);
	}
	@Test
	void testUpdateClaim_ValidData() throws Exception{
		Long id = 1L;
		PolicyMaster policyObj = new PolicyMaster(id, "POL_1", "medical", 100, 1000);
		UserPolicyClaim userClaim = new UserPolicyClaim(id, new Date(), 500, "success", new Date(), policyObj);
	...
	void testUpdateClaim_InvalidPolicyException() throws Exception{
		...
		when(userPolicyClaimRepo.save(userClaim)).thenReturn(userClaim);
		when(policyMasterRepository.save(policyObj)).thenReturn(policyObj);
		InvalidPolicyException thrown = assertThrows(InvalidPolicyException.class, ()->service.updateClaim(id, userClaim, token));
	...
	void testUpdateClaim_ClaimNotFoundException() throws Exception{
		PolicyMaster pObj = new PolicyMaster(1L, "POL_1", "medical", 100, 1000);
		UserPolicyClaim claimObj = new UserPolicyClaim(1L, new Date(), 500, "success", new Date(), pObj);
		String token = "Bearer token";
		when(policyMasterRepository.findById(1L))
				.thenReturn(Optional.of(new PolicyMaster(1L, "POL_1", "medical", 100, 1000)));
		when(userPolicyClaimRepo.findById(1L))
				.thenReturn(Optional.of(new UserPolicyClaim(1L, new Date(), 5000, "success", new Date(), pObj)));
		when(walletClient.claimDeposit(token, new Transaction())).thenReturn((double) 100);
