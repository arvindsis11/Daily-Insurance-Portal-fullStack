...
class PolicyServiceImplTest {
	...
	void testCreatePolicy_NotNull_ValidData() {
		PolicyMaster pObj = new PolicyMaster(1L, "POL_1", "medical", 100, 0);
		String token = "token";
		when(walletClient.getTotalBalance(token)).thenReturn((double) 1000);
	...
	void testCreatePolicy_InvalidData() {
		PolicyMaster pObj = new PolicyMaster(1L, "POL_1", "medical", 1000, 0);
		String token = "token";
		when(walletClient.getTotalBalance(token)).thenReturn((double) 100);
		when(policyMasterRepo.save(pObj)).thenReturn(pObj);
	...
	void testViewAllPolicy() {
		when(policyMasterRepo.findAll()).thenReturn(Stream.of(new PolicyMaster(1L, "POL_1", "medical", 100, 1000),new PolicyMaster(1L, "POL_1", "medical", 500, 5000)).collect(Collectors.toList()));
		assertThat(service.viewPolicy().size()).isEqualTo(2);
