...
class PolicyControllerTest {
	...
	void testMyPolicy_ValidToken() throws Exception {
		PolicyMaster pObj = new PolicyMaster(1L, "POL_1", "Health", 100, 0);
		String jsonObj = this.mapToJson(pObj);
		when(authclient.authorize("test")).thenReturn(true);
		when(walletClient.getTotalBalance("test")).thenReturn((double) 500);
	...
	@Test
	void testCreateConsumerClient_ValidToken() throws Exception {
		Consumer cObj = new Consumer(1L, "Arvind", "Sisodiya", "arvindsis35@gmail.com");
		String jsonObj = this.mapToJson(cObj);
		when(authclient.authorize("test")).thenReturn(true);
		MvcResult mvres = mockMvc.perform(post("/policy/create-consumer").header("Authorization", "test").contentType("application/json").content(jsonObj)).andReturn();
		assertThat(mvres.getResponse().getStatus()).isEqualTo(200);
	}
	
	@Test
	void testCreateConsumerClient_InValidToken() throws Exception {
		Consumer cObj = new Consumer(1L, "Arvind", "Sisodiya", "arvindsis35@gmail.com");
		String jsonObj = this.mapToJson(cObj);
		when(authclient.authorize("test")).thenReturn(false);
		MvcResult mvres = mockMvc.perform(post("/policy/create-consumer").header("Authorization", "test").contentType("application/json").content(jsonObj)).andReturn();
	...
	@Test
	void testViewConsumerClient_ValidToken() throws Exception {
		Consumer cObj = new Consumer(1L, "Arvind", "Sisodiya", "arvindsis35@gmail.com");
		String jsonObj = this.mapToJson(cObj);
		when(authclient.authorize("test")).thenReturn(true);
		MvcResult mvres = mockMvc.perform(get("/policy/view-consumer").header("Authorization", "test")
				.contentType("application/json").content(jsonObj)).andReturn();
		assertThat(mvres.getResponse().getStatus()).isEqualTo(200);
	...
	@Test
	void testViewConsumerClient_InValidToken() throws Exception {
		Consumer cObj = new Consumer(1L, "Arvind", "Sisodiya", "arvindsis35@gmail.com");
		String jsonObj = this.mapToJson(cObj);
		when(authclient.authorize("test")).thenReturn(false);
		MvcResult mvres = mockMvc.perform(get("/policy/view-consumer").header("Authorization", "test")
	...
	void testUpdateUserPolicyClaim_ValidToken() throws Exception {
		PolicyMaster pObj = new PolicyMaster(1L, "POL_1", "Health", 100, 0);
		Date date = new Date();
		UserPolicyClaim claimObj = new UserPolicyClaim(1L, date, 1000, "dummy", date, pObj);
