...
public class PolicyController {
	...
	public List<PolicyMaster> viewPolicy(@RequestHeader("Authorization") String token) throws AuthorizationException {
		if (authorizeClient.authorize(token)) {
	...
	@ApiOperation(notes = "returns PolicyMaster object, details", value = "display the policy created")
	public Consumer createConsumerClient(@RequestBody Consumer consumer, @RequestHeader("Authorization") String token)
			throws AuthorizationException {
		if (authorizeClient.authorize(token)) {
	...
	@ApiOperation(notes = "returns Consumer object, details", value = "display all available Consumers")
	public List<Consumer> viewConsumerClient(@RequestHeader("Authorization") String token)
			throws AuthorizationException {
		if (authorizeClient.authorize(token)) {
			logger.debug("view all Consumers");
			return consumerService.viewConsumer();
		} else {
			logger.error("Access Denied");
			throw new AuthorizationException("Access Denied");
		}
	...
	public UserPolicyClaim updateUserPolicyClaim(@PathVariable(value="id") Long id, @RequestBody UserPolicyClaim claim,
	...
	@GetMapping("/view-claim")
	@ApiOperation(notes = "returns UserPolicyClaim object, details", value = "display all claims raised")
	public List<UserPolicyClaim> viewUserPolicyClaim(@RequestHeader("Authorization") String token)
			throws AuthorizationException {
		if (authorizeClient.authorize(token)) {
			logger.debug("view all claims");
			return userPolicyClaimService.viewClaim();
		} else {
			logger.error("Access Denied");
			throw new AuthorizationException("Access Denied");
		}
