...
class WalletServiceImplTest {
	...
	void testCreateWalletNotnull() throws Exception{
		Wallet walletObj = new Wallet(1L,"paytm",100);
		Optional<Transaction> transObj = Optional.of(new Transaction(1L,100));
	...
	void testGetTotalBalance() {
		...
		Optional<Transaction> transObj = Optional.of(new Transaction(1L,100));
		when(transrepo.findById(1L)).thenReturn(transObj);
		assertThat(service.getTotalBalance()).isEqualTo(100);
	}

	@Test
	void testFindAllWallets() {
		when(walletrepo.findAll()).thenReturn(Stream.of(new Wallet(1L,"paytm",100),new Wallet(2L,"phonepe",300)).collect(Collectors.toList()));
		assertThat(service.findAllWallets().size()).isEqualTo(2);
	}
	...
	void testDeposit() {
		Optional<Transaction> transObj = Optional.of(new Transaction(1L,100));
		when(transrepo.findById(1L)).thenReturn(transObj);
		assertThat(service.deposit(100)).isEqualTo(200);
	}
	...
	void testWithdraw() {
		Optional<Transaction> transObj = Optional.of(new Transaction(1L,100));
		when(transrepo.findById(1L)).thenReturn(transObj);
		assertThat(service.withdraw(50)).isEqualTo(50);
	}
	...
	void testPolicyWithdraw() {
		Optional<Transaction> transObj = Optional.of(new Transaction(1L,100));
		when(transrepo.findById(1L)).thenReturn(transObj);
		assertThat(service.policyWithdraw(20)).isEqualTo(80);
	...
	void testClaimDeposit() {
		Optional<Transaction> transObj = Optional.of(new Transaction(1L,100));
		when(transrepo.findById(1L)).thenReturn(transObj);
		assertThat(service.claimDeposit(150)).isEqualTo(250);
	}
