...
class WalletRepositoryTest {
	...
	@BeforeEach
	void setup() {
		walletObj = new Wallet(1L,"Paytm",1000);
	}
	...
	@Test
	void testFindAll() {
		when(repo.findAll()).thenReturn(Stream.of(walletObj,new Wallet(2L,"phonePe",100)).collect(Collectors.toList()));
		assertThat(repo.findAll().size()).isEqualTo(Stream.of(walletObj,new Wallet(2L,"phonePe",100)).collect(Collectors.toList()).size());
	}
